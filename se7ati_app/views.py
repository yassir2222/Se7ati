...
def doctor_articles(request):
    ...
    if request.method == 'POST':
        ...
        if title and content:
            article = Article(title=title, content=content, doctor=request.user)
            article.save()
            return redirect('doctor_articles')  # Redirect to the articles page after saving
        else:
            # Handle the case where title or content is missing (optional: display an error message)
            pass

    # Get all articles created by the logged-in doctor
    articles = Article.objects.filter(doctor=request.user).order_by('-created_at')
    context = {'articles': articles}
    return render(request, 'doctor/articles.html', context)

@login_required
def update_article(request, article_id):
    article = get_object_or_404(Article, pk=article_id)

    # Check if the user is a doctor and the owner of the article
    if not request.user.is_authenticated or request.user.user_type != 'doctor' or article.doctor != request.user:
    ...
    if request.method == 'POST':
        ...
        if title and content:
            ...
            article.save()
            return redirect('doctor_articles')
        else:
            # Handle the case where title or content is missing
            messages.error(request, "Title and content are required.")
    
    return render(request, 'doctor/update_article.html', {'article': article})
...
def delete_article(request, article_id):
    ...
    # If it's a GET request (e.g., confirmation page), you might want to render a confirmation template
    return render(request, 'doctor/delete_article.html', {'article': article})

@login_required
def patient_articles(request):
    ...
    context = {'articles': articles}
    return render(request, 'patient/articles.html', context)
